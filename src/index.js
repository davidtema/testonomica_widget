...
window.tnc = function (id, tag) {
    ...
    //     $.ajax(`/test/save/?testId=${id}&key=${e.key}`).then((response) => {
    //         window.location = `/test/result/?code=${response.key}`;
    //     }).fail(() => {
    //         alert('Не удалось сохранить прогресс.');
    //     });
    // });

    runner.addEventListener('log', function (e) {
        console.log(...e.msg);
    });
...
// import App from "testonomica_api/src/site/App";
